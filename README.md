...
4. Na nuvem, um único escopo `ativo|timeframe` é treinado por hora para permanecer dentro do orçamento gratuito. Além do holdout cronológico de pelo menos 300 observações, o artefato precisa passar três janelas walk-forward expansivas, com purga temporal, cobertura mínima e EV por oportunidade não negativo em todas elas. Cada janela estima a taxa de empate apenas com o próprio período de treino, com suavização de Laplace; resultados futuros não entram nesse gate.
...
A implantação atual requer todas as migrations `001`–`033` presentes na pasta. As migrations `001`–`027` mantêm o ledger causal, treino, M30, calendário, qualidade desacoplada e política única. A `028` corrige a reconciliação em lote de lacunas sem reutilizar estado antigo; `029/030` criam seis controles prospectivos independentes; a `031` publica baselines ingênuos, Wilson por nota, diagnóstico da nota A e persiste o regime causal; a `032` compara cada braço com um acaso de cobertura idêntica; e a `033` publica o diagnóstico contextual da nota A por sessão UTC, fonte e latência. Uma reconstrução parcial fica incompatível com o frontend atual.
